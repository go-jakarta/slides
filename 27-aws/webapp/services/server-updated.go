...
func NewServer(db *sql.DB, logf func(string, ...interface{})) *Server {
	...
	s.HandleFunc(goji.Get("/books/:isbn"), s.GetBooksByIsbn)
	return s
}
...
func (s *Server) GetBooksByIsbn(res http.ResponseWriter, req *http.Request) {
	isbn := goji.Param(req, "isbn")
	if isbn == "" {
		jsonerrorf(res, http.StatusBadRequest, "missing isbn")
		return
	}
	books, err := models.BookByIsbn(s.db, isbn)
	if err != nil {
		s.logf("error: %v", err)
		jsonerrorf(res, http.StatusNotFound, "not found")
		return
